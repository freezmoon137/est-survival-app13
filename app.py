...
class ExtraSurvivalTrees:
    ...
    def predict_survival_function(self, X, return_array=False):
        ...
        survival_functions = self.model.predict_survival_function(X, return_array=return_array)
    ...
    def predict_cumulative_hazard_function(self, X, return_array=False):
        ...
        hazard_functions = self.model.predict_cumulative_hazard_function(X, return_array=return_array)
...
def create_default_model():
    ...
    data = {
        ...
        'Hemoglobin': np.random.normal(130, 15, n_samples),
        'Creatinine': np.random.normal(80, 20, n_samples),
        'Urea': np.random.normal(6, 2, n_samples),
        'Sodium': np.random.normal(140, 3, n_samples),
        'Potassium': np.random.normal(4, 0.5, n_samples),
        'Coronary_Heart_Disease': np.random.binomial(1, 0.3, n_samples),
        'Diabetes': np.random.binomial(1, 0.25, n_samples),
        'Hypertension': np.random.binomial(1, 0.4, n_samples),
        'Stroke': np.random.binomial(1, 0.15, n_samples),
        'Malignancy': np.random.binomial(1, 0.2, n_samples),
        'COPD': np.random.binomial(1, 0.1, n_samples),
        'CKD': np.random.binomial(1, 0.3, n_samples),
        'Liver_Disease': np.random.binomial(1, 0.1, n_samples),
        'Dementia': np.random.binomial(1, 0.05, n_samples),
        'Depression': np.random.binomial(1, 0.15, n_samples)
    }
...
def predict_survival_risk(data: Dict[str, Any], prediction_time: int = 180) -> Dict[str, Any]:
    ...
    try:
        ...
        # 特征重要性（模拟）
        feature_importance = {}
        if hasattr(est_model, 'model') and hasattr(est_model.model, 'feature_importances_'):
            importances = est_model.model.feature_importances_
